# The 4-hourly wiki-recap timer and the fish greeting that shows its brief. Import it
# on the machines that should have them; the wiki-recap command itself comes from
# ../default.nix on every machine.
{ config, ... }:
{
  # Every 4 hours, regenerate the wiki recap if the wiki changed (claude runs only then).
  ...
  systemd.user = {
    ...
    timers.wiki-recap = {
      Unit.Description = "Regenerate the claude-wiki recap every 4 hours";
      Timer = {
